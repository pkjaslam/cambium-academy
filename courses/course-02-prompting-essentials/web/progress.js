...
(function(){
  ...
  function render(){
    var slides = read("cambium-c02-slides") || {};
    var seen = slides.seen || 0;
    var watched = !!(read("cambium-c02-lecture") || {}).watched;
    var lectureDone = watched || seen >= 40;
    var cards = Object.keys(read("cambium-c02-cards") || {}).length;
    var pg = read("cambium-c02-playground") || {};
    var pgN = (pg.tok?1:0) + (pg.lm?1:0) + (pg.net?1:0);
    var quiz = read("cambium-c02-quiz") || {};
    var cert = read("cambium-c02-cert") || {};
    var unlocked = lectureDone && cards >= 24 && pgN >= 3;

    var steps = [
      { label: "1 · Lecture", detail: lectureDone ? (watched && seen < 40 ? "watched the video" : "done") : (seen > 0 ? seen + " of 40 slides" : "slides or video"), pct: lectureDone ? 100 : Math.min(99, Math.round(100 * seen / 40)), href: "slides.html" },
  ...
  if (btn) {
    var w = !!(read("cambium-c02-lecture") || {}).watched;
    if (w) { btn.textContent = "Lecture marked as watched"; btn.style.background = "#97BC62"; btn.style.color = "#1F3D24"; }
    btn.addEventListener("click", function(){
      localStorage.setItem("cambium-c02-lecture", JSON.stringify({ watched: true, date: new Date().toISOString().slice(0,10) }));
