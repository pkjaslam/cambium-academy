...
(function(){
  ...
  function render(){
    ...
    var quiz = read("cambium-c03-quiz") || {};
    var cert = read("cambium-c03-cert") || {};
  ...
  if (btn) {
    var w = !!(read("cambium-c03-lecture") || {}).watched;
    if (w) { btn.textContent = "Lecture marked as watched"; btn.style.background = "#97BC62"; btn.style.color = "#1F3D24"; }
    btn.addEventListener("click", function(){
      localStorage.setItem("cambium-c03-lecture", JSON.stringify({ watched: true, date: new Date().toISOString().slice(0,10) }));
