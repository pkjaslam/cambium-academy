...
(function(){
  var COURSE = "Research with AI (Cambium Academy Course 03)";
  var ORG_ID = "133384582"; // linkedin.com/company/cambium-ai-institute
  var LANDER = "https://pkjaslam.github.io/cambium-academy/start/";
  var CERTURL = "https://pkjaslam.github.io/cambium-academy/courses/course-03-research-with-ai/web/certificate.html";
  ...
  var timer = setInterval(function(){
    ...
    var m = (document.body.innerText || "").match(/CAMB-C03-[A-Z0-9]{6}/);
  ...
  function init(cert, certId){
    ...
    document.getElementById("badgeBtn").onclick = function(){
      ...
      function bakeOpenBadge(canvas){
        var BASE = "https://pkjaslam.github.io/cambium-academy/courses/course-03-research-with-ai/web/";
        ...
        var assertion = {
          ...
          "badge": BASE + "badges/course-03-research-with-ai.json",
          "issuedOn": issued,
    ...
    setTimeout(function(){
      try {
        ...
        if (localStorage.getItem("cambium-c03-registered") === certId) return;
        var learner = rd("cambium-c03-learner") || {};
        fetch(reg.endpoint, {
          method: "POST",
          mode: "no-cors",
          headers: { "Content-Type": "text/plain" },
          body: JSON.stringify({ course: "C03", name: cert.name, org: cert.org || "", email: learner.email || "", certId: certId, date: cert.date || "", score: cert.score || "" })
        }).then(function(){ localStorage.setItem("cambium-c03-registered", certId); }).catch(function(){});
      } catch(e) {}
    }, 800);
  }
})();
