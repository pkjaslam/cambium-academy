...
(function(){
  ...
  var PAGES = {
    "index.html": { k: "page-index", t: "This is the course home. Read the slides or watch the lecture, and keep an eye on your progress bar right on this page. When you are ready, I will walk you to the quiz." },
    "slides.html": { k: "page-slides", t: "These are the full course slides: twelve modules on building agents that work and cannot hurt you. Use the arrow keys or swipe. I will pop a two-question check after each module so nothing slips past you." },
    ...
    "playground.html": { k: "page-playground", t: "My favorite room. Scope tasks into automate, assist, or never; wire jobs to the right tool; hunt a poisoned page an agent obeyed; catch a regression a working agent hid; and assign least-privilege permissions before it runs. All five games count toward unlocking the quiz." },
    ...
    "capstone.html": { k: "page-capstone", t: "The capstone is three artifacts you would really ship: one real automation, its eval (a golden set with today's pass rate), and its runbook. Do this one; future you says thanks." },
    "career.html": { k: "page-career", t: "Let us make this course count. Copy the resume lines, add the certificate to LinkedIn, and rehearse the agent-and-safety questions until you sound like yourself on a good day." },
    "transcript.html": { k: "page-transcript", t: "The full lecture, word for word. That voice in the video, by the way, is me." },
    "practical.html": { k: "page-practical", t: "The Run Review is the honors exam: you audit one real agent run, find the poisoned instruction it obeyed and the step it skipped, name the permission that made it an incident, and write the fix. Pass it and your certificate carries the SHIP-VERIFIED stamp." },
    "practice.html": { k: "page-practice", t: "The Practice Range is practice with known answers: score a golden set, sort an agent's permissions into grant or gate, and repair a broken runbook. Warm up here before the Practical." },
    "pack.html": { k: "page-pack", t: "The Builder's Pack is yours to keep: a live agent design canvas that scores your plan for the lethal trifecta and missing gates, a golden-set template, the runbook skeleton, and every tool and standard, all copy-ready and printable." },
    ...
    "foundry.html": { k: "page-foundry", t: "The Agent Foundry is where you stop reading about agents and build one. Five slots: goal, brain, tools, memory, stop rules. Give it real tools with real scopes, then watch every move it makes on the live trace. When it wants a permission you did not grant, it stops and asks you. Run the injection drill twice, naive then scoped, and you will never forget the difference." },
    "workshop.html": { k: "page-workshop", t: "The Workshop is the course you can teach to other people. A ninety minute run sheet, three persona builds for faculty, administrators, and researchers, printable recipe cards, and a crib sheet for everything that will go wrong in the room. Take it to your department." },
    "sandbox.html": { k: "page-sandbox", t: "This is the page I am proudest of. You configure an agent's tools and gates, then run it against a poisoned input, right here in your browser. If you over-permissioned it, you watch the injection steal data. If you gave least privilege and a human gate, you watch the same attack hit a wall and die. That contrast, on one screen, is the whole course." }
  };
  var GENERIC = { k: "page-generic", t: "Ask me what to do next and I will check your progress." };

  function read(k){ try { return JSON.parse(localStorage.getItem(k) || "null"); } catch(e){ return null; } }
  function advise(){
    var s = read("cambium-c05-slides") || {}, seen = s.seen || 0;
    var lecture = seen >= 79 || !!(read("cambium-c05-lecture") || {}).watched;
    ...
    var quiz = read("cambium-c05-quiz") || {}, cert = read("cambium-c05-cert") || {};
    if (cert.issued) return { k: "next-certdone", t: "You have finished Course 05, Building with AI, certificate and all. Try the Practical for the honors stamp, post your certificate on the graduate wall, and I will see you next week for the next Cambium Academy course.", href: "practical.html", label: "Take the Practical" };
    ...
    if (!lecture) {
      if (seen > 0) return { k: "next-slides-continue", t: "Step 1 of your path: the lecture. You are " + seen + " slides in, out of 79. Keep going; the next module is shorter than you think.", href: "slides.html#" + Math.min(seen + 1, 79), label: "Continue the slides" };
