...
(function(){
  var COURSE = "Responsible AI in Scholarly Publishing (Cambium Academy Course 04)";
  var ORG_ID = "133384582"; // linkedin.com/company/cambium-ai-institute
  var LANDER = "https://pkjaslam.github.io/cambium-academy/start/";
  var CERTURL = "https://pkjaslam.github.io/cambium-academy/courses/course-04-responsible-ai-publishing/web/certificate.html";
  ...
  var timer = setInterval(function(){
    ...
    var m = (document.body.innerText || "").match(/CAMB-C04-[A-Z0-9]{6}/);
  ...
  function init(cert, certId){
    ...
    var shareText = "I just earned the Responsible AI in Scholarly Publishing certificate from Cambium AI Institute: the 2026 rules for authors, reviewers, and editors. Disclose properly, referee without breaking confidentiality, and never accuse anyone on an AI-detector score. Free, hands-on, and the quiz stays locked until you actually study. Join in: " + LANDER;
    ...
    document.getElementById("badgeBtn").onclick = function(){
      ...
      function finish(){
        ...
        c.fillText("RESEARCH WITH AI", cx, cy + 40);
        ...
        try {
          ...
          a.download = "cambium-academy-research-with-ai-badge.png";
      ...
      function bakeOpenBadge(canvas){
        var BASE = "https://pkjaslam.github.io/cambium-academy/courses/course-04-responsible-ai-publishing/web/";
        ...
        var assertion = {
          ...
          "badge": BASE + "badges/course-04-responsible-ai-publishing.json",
          "issuedOn": issued,
    ...
    setTimeout(function(){
      try {
        ...
        if (localStorage.getItem("cambium-c04-registered") === certId) return;
        var learner = rd("cambium-c04-learner") || {};
        fetch(reg.endpoint, {
          method: "POST",
          mode: "no-cors",
          headers: { "Content-Type": "text/plain" },
          body: JSON.stringify({ course: "C04", name: cert.name, org: cert.org || "", email: learner.email || "", certId: certId, date: cert.date || "", score: cert.score || "" })
        }).then(function(){ localStorage.setItem("cambium-c04-registered", certId); }).catch(function(){});
      } catch(e) {}
    }, 800);
  }
})();
