...
// lecture -> flashcards -> AI Lab (6 games) -> quiz (locked until all are done) -> certificate.
(function(){
  function read(k){ try { return JSON.parse(localStorage.getItem(k) || "null"); } catch(e){ return null; } }
  function render(){
    var slides = read("cambium-c04-slides") || {};
    var seen = slides.seen || 0;
    var watched = !!(read("cambium-c04-lecture") || {}).watched;
    var lectureDone = watched || seen >= 80;
    ...
    var quiz = read("cambium-c04-quiz") || {};
    var cert = read("cambium-c04-cert") || {};
    var unlocked = lectureDone && cards >= 24 && pgN >= 6;

    var steps = [
      { label: "1 · Lecture", detail: lectureDone ? (watched && seen < 80 ? "watched the video" : "done") : (seen > 0 ? seen + " of 80 slides" : "slides or video"), pct: lectureDone ? 100 : Math.min(99, Math.round(100 * seen / 80)), href: "slides.html" },
      { label: "2 · Flashcards", detail: cards + " of 24 known", pct: Math.min(100, Math.round(100 * cards / 24)), href: "flashcards.html" },
      { label: "3 · AI Lab", detail: pgN + " of 6 games won", pct: Math.round(100 * pgN / 6), href: "playground.html" },
  ...
  if (btn) {
    var w = !!(read("cambium-c04-lecture") || {}).watched;
    if (w) { btn.textContent = "Lecture marked as watched"; btn.style.background = "#97BC62"; btn.style.color = "#1F3D24"; }
    btn.addEventListener("click", function(){
      localStorage.setItem("cambium-c04-lecture", JSON.stringify({ watched: true, date: new Date().toISOString().slice(0,10) }));
