...
(function(){
  ...
  var PAGES = {
    ...
    "slides.html": { k: "page-slides", t: "These are the full course slides: twelve modules, three chairs. Use the arrow keys or swipe. I will pop a two-question check after each module so nothing slips past you." },
    "quiz.html": { k: "page-quiz", t: "The quiz unlocks after the lecture, all twenty-four flashcards, and the six AI Lab games. Then it is twenty questions, fourteen to pass. Ask me what is left and I will tell you." },
    ...
    "playground.html": { k: "page-playground", t: "My favorite room. Place disclosures correctly, sort assists from misconduct, hunt ghost citations against the live registry, find real red flags, uncover a hidden prompt, and survive the detector trap. All six games count toward unlocking the quiz." },
    ...
    "capstone.html": { k: "page-capstone", t: "The capstone is three artifacts you would really sign: a disclosure statement for your next paper, a one-page AI policy, and an honest audit of something you already published. Do this one; future you says thanks." },
    "career.html": { k: "page-career", t: "Let us make this course count. Copy the resume lines, add the certificate to LinkedIn, and rehearse the integrity questions until you sound like yourself on a good day." },
    "transcript.html": { k: "page-transcript", t: "The full lecture, word for word. That voice in the video, by the way, is me." },
    "practical.html": { k: "page-practical", t: "The Editor's Desk is the honors exam: one manuscript packet, a live registry check, a hidden prompt to find, real red flags to name, and a decision to write. Pass it and your certificate carries the DESK-VERIFIED stamp." },
    "practice.html": { k: "page-practice", t: "The Screening Room is practice with known answers: a reference list with planted fakes, a hollow review to grade, and a disclosure statement to repair. Warm up here before the Practical." },
    "pack.html": { k: "page-pack", t: "The Author's Pack is yours to keep: the disclosure builder, per-venue statements, the reviewer and editor checklists, and the policy links, all copy-ready and printable." },
    "classroom.html": { k: "page-classroom", t: "This is my classroom. Ask about any lesson, request an example, or have me draw a diagram. I already know how far you are and what you keep missing." },
    "referee.html": { k: "page-referee", t: "This is the page I am proudest of. It loads a language model into your own browser, so you can put a confidential manuscript in front of an AI without it ever leaving your machine. A counter on the page proves it. Note that if you ask me a question here, that counter will tick up, because I am a cloud AI and the referee is not. That is the lesson." },
    "auditor.html": { k: "page-auditor", t: "Paste your own reference list here and every DOI gets resolved against the live Crossref registry. Run it on your next manuscript before a reviewer does; most people find at least one casualty." }
  };
  var GENERIC = { k: "page-generic", t: "Ask me what to do next and I will check your progress." };

  function read(k){ try { return JSON.parse(localStorage.getItem(k) || "null"); } catch(e){ return null; } }
  function advise(){
    var s = read("cambium-c04-slides") || {}, seen = s.seen || 0;
    var lecture = seen >= 80 || !!(read("cambium-c04-lecture") || {}).watched;
    var cards = Object.keys(read("cambium-c04-cards") || {}).length;
    var pg = read("cambium-c04-playground") || {};
    var pgN = (pg.disc?1:0) + (pg.line?1:0) + (pg.ghost?1:0) + (pg.flags?1:0) + (pg.inject?1:0) + (pg.trap?1:0);
    var quiz = read("cambium-c04-quiz") || {}, cert = read("cambium-c04-cert") || {};
    if (cert.issued) return { k: "next-certdone", t: "You have finished Course 04, Responsible AI in Scholarly Publishing, certificate and all. Try the Practical for the honors stamp, post your certificate on the graduate wall, and I will see you next week for the next Cambium Academy course.", href: "practical.html", label: "Take the Practical" };
    ...
    if (lecture && cards >= 24 && pgN >= 6) return { k: "next-quizready", t: "Lecture, flashcards, and all six games: done. The quiz is unlocked" + (quiz.best ? ", and your best so far is " + quiz.best + " out of 20; fourteen passes." : ". Twenty questions, fourteen to pass. You are ready."), href: "quiz.html", label: "Start the quiz" };
    if (!lecture) {
      if (seen > 0) return { k: "next-slides-continue", t: "Step 1 of your path: the lecture. You are " + seen + " slides in, out of 80. Keep going; the next module is shorter than you think.", href: "slides.html#" + Math.min(seen + 1, 80), label: "Continue the slides" };
    ...
    return { k: "next-playground", t: "Almost there. Step 3: the AI Lab. You have won " + pgN + " of 6 games. Each stamp counts toward the quiz.", href: "playground.html", label: "Open the AI Lab" };
  }
